...
>([
  ...
  [
    'minute',
    {
      ...
      landingAssertion:
        'Visiting MinuteLoginUrl (?key=<token>) lands on the clean URL after the CloudFront Function consumes the token and 302s. Title matches /Minute/.',
      loginAssertion:
        'After magic-link redirect, page URL does NOT contain key=; cookie is set; same-origin fetch() succeeds.',
      featureFlow:
        'Inside the SPA, fetch() against the same origin must succeed (catches the magic-link cookie-path regression that replaces the old basic-auth-breaks-fetch() flow). /api/proxy/healthcheck must reach the backend via frontend middleware (catches the ALB /api/* rule interception regression).',
      historicalRegressionCited:
        'Minute had 12 issues including basic-auth breaking fetch() and corporate browsers stripping URL-embedded credentials. On 2026-05-11 the scenario pivoted to magic-link + 7-day HttpOnly cookie (commit 6387441). See memory:minute-scenario-lessons.md.',
      quarantine: { state: 'active' },
    },
  ],
  [
