...
The `quarterly-audit.yml` workflow (Phase 3 task T3.8b) opens a tracking issue
every 3 months. The audit covers:

- Smoke-account spend (`aws ce get-cost-and-usage`)
- Orphan-resource sweep (resources without matching active CFN stacks)
- Deploy-role policy actuals vs runbook-committed JSON (drift detection)
- SCP attachments live vs `$CONFIG_FILE.expected_scps`
- Renovate liveness check

If the audit issue stays open >30 days, an auto-escalation step posts a comment
nudging the assignee.
...
- **Owner**: the operator who initially set up Renovate (recorded in the
  quarterly-audit issue body when first created).
...
- **Failure mode**: when the token expires, Renovate stops opening PRs but does
  not error visibly. The quarterly-audit "Renovate liveness check" (queries
  `gh pr list --author renovate[bot] --search 'created:>30d ago'`) catches this.
...
automated budget alarm in this account; the quarterly-audit issue is the
backstop.
