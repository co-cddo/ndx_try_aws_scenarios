...
### Step 13: ai-contact-centre PSTN handling

No real PSTN is claimed for smoke. The smoke deploy passes a DUMMY value for
`AiccExistingPhoneNumberArn` (see `aicc_existing_phone_number_arn` in
`docs/smoke-test-account-config.yml`); the ai-contact-centre template's
`ClaimNewPhoneNumber` condition is `!Equals [!Ref ExistingPhoneNumberArn, '']`
so any non-empty string skips the `GeoNumber` claim and the `GeoFlowAssoc`
custom resource. AICC's ConnectInstance + Lex + Wisdom + KB + companion UI
all still deploy real; the smoke spec checks the companion URL HTTP status
and that `PstnNumber` matches a `+44…` format string ("+442012345678" by
default).

UK DID claim quota is **5/30 days**. Even a single release+reclaim per day
exceeds it, so we preserve the quota entirely for production deploys. No
operator setup required for this step.
