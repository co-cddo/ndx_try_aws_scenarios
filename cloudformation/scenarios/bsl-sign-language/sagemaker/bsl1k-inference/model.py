"""
I3D (Inflated 3D ConvNet) model architecture.
Exact copy of github.com/gulvarol/bsl1k/models/i3d.py — no modifications.
"""
import math
...
class InceptionI3d(nn.Module):
    """Inception-v1 I3D architecture — exact match of BSL-1K original."""
    ...
    def __init__(self, num_classes=400, spatiotemporal_squeeze=True,
        ...
        # Plain dict — NOT nn.ModuleDict (matches original BSL-1K code)
        self.end_points = {}

        end_point = 'Conv3d_1a_7x7'
        self.end_points[end_point] = Unit3D(
            in_channels=in_channels, output_channels=64, kernel_shape=[7, 7, 7],
        ...
        end_point = 'Mixed_4b'
        self.end_points[end_point] = InceptionModule(
            480, [192, 96, 208, 16, 48, 64], name + end_point)
        if self._final_endpoint == end_point:
            self.build()
            return
        ...
        end_point = 'Mixed_4c'
        self.end_points[end_point] = InceptionModule(
            512, [160, 112, 224, 24, 64, 64], name + end_point)
        if self._final_endpoint == end_point:
            self.build()
            return
        ...
        end_point = 'Mixed_4d'
        self.end_points[end_point] = InceptionModule(
            512, [128, 128, 256, 24, 64, 64], name + end_point)
        if self._final_endpoint == end_point:
            self.build()
            return
        ...
        end_point = 'Mixed_4e'
        self.end_points[end_point] = InceptionModule(
            512, [112, 144, 288, 32, 64, 64], name + end_point)
        if self._final_endpoint == end_point:
            self.build()
            return
        ...
        end_point = 'Mixed_4f'
        self.end_points[end_point] = InceptionModule(
            528, [256, 160, 320, 32, 128, 128], name + end_point)
        if self._final_endpoint == end_point:
            self.build()
            return
        ...
        end_point = 'Mixed_5b'
        self.end_points[end_point] = InceptionModule(
            832, [256, 160, 320, 32, 128, 128], name + end_point)
        if self._final_endpoint == end_point:
            self.build()
            return
        ...
        end_point = 'Mixed_5c'
        self.end_points[end_point] = InceptionModule(
            832, [384, 192, 384, 48, 128, 128], name + end_point)
        if self._final_endpoint == end_point:
            self.build()
            return

        # Logits — stored as self.logits, NOT in end_points (matches original)
        last_duration = int(math.ceil(num_in_frames / 8))
        last_size = 7
        self.avgpool = nn.AvgPool3d((last_duration, last_size, last_size), stride=1)
        self.dropout = nn.Dropout(dropout_keep_prob)
        self.logits = Unit3D(
            in_channels=1024, output_channels=self._num_classes,
            kernel_shape=[1, 1, 1], padding=0, activation_fn=None,
