# ISB Blueprint Registration: Planning Application AI

Register the NDX:Try Planning Application AI scenario as an Innovation Sandbox (ISB) blueprint so it auto-deploys into sandbox accounts when leases are approved.
...
aws cloudformation create-stack-set \
  --stack-set-name ndx-try-planning-ai \
  --template-url https://{BUCKET}.s3.{REGION}.amazonaws.com/scenarios/planning-ai/template.yaml \
...
3. In the blueprint selection step, select `ndx-try-planning-ai`
4. Save the lease template

## Verification

1. Request a test lease using the lease template from Step 4
2. Wait for lease approval and blueprint deployment (should complete within 8 minutes)
3. Check the sandbox account for:
   - Lambda function `ndx-try-planning-analyzer-{region}`
