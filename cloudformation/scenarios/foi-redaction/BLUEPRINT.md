# ISB Blueprint Registration: FOI Redaction

Register the NDX:Try FOI Redaction scenario as an Innovation Sandbox (ISB) blueprint so it auto-deploys into sandbox accounts when leases are approved.
...
aws s3 cp template.yaml \
  s3://{BUCKET}/scenarios/foi-redaction/template.yaml
```
...
aws cloudformation create-stack-set \
  --stack-set-name ndx-try-foi-redaction \
  --template-url https://{BUCKET}.s3.{REGION}.amazonaws.com/scenarios/foi-redaction/template.yaml \
  ...
  --description "NDX:Try FOI Redaction - Automated sensitive data redaction for FOI requests"
```
...
2. Enter name: `ndx-try-foi-redaction` (must be 1-50 chars, start with letter, alphanumeric + hyphens only)
3. Select the `ndx-try-foi-redaction` StackSet
4. Configure deployment:
   - Select target regions (template works in any region with Amazon Comprehend)
   - Set timeout: **10 minutes** recommended for this template (deploys in 4-6 minutes)
5. Review and submit
...
3. In the blueprint selection step, select `ndx-try-foi-redaction`
4. Save the lease template

## Verification

1. Request a test lease using the lease template from Step 4
2. Wait for lease approval and blueprint deployment (should complete within 6 minutes)
3. Check the sandbox account for:
   - Lambda function `ndx-try-foi-redactor-{region}`
   - S3 bucket `ndx-try-foi-docs-{account-id}-{region}`
   - IAM role `ndx-try-foi-role-{region}`
4. Open the Lambda Function URL and verify the redaction interface loads and processes sample text
