# ISB Blueprint Registration: QuickSight Dashboard

Register the NDX:Try QuickSight Dashboard scenario as an Innovation Sandbox (ISB) blueprint so it auto-deploys into sandbox accounts when leases are approved.
...
aws s3 cp template.yaml \
  s3://{BUCKET}/scenarios/quicksight-dashboard/template.yaml
```
...
aws cloudformation create-stack-set \
  --stack-set-name ndx-try-quicksight-dashboard \
  --template-url https://{BUCKET}.s3.{REGION}.amazonaws.com/scenarios/quicksight-dashboard/template.yaml \
  ...
  --description "NDX:Try QuickSight Dashboard - Service performance analytics and reporting"
```
...
2. Enter name: `ndx-try-quicksight-dashboard` (must be 1-50 chars, start with letter, alphanumeric + hyphens only)
3. Select the `ndx-try-quicksight-dashboard` StackSet
4. Configure deployment:
   - Select target regions (template works in any region with Amazon QuickSight)
   - Set timeout: **15 minutes** recommended (QuickSight subscription and resource provisioning take longer)
5. Review and submit
...
3. In the blueprint selection step, select `ndx-try-quicksight-dashboard`
...
3. Check the sandbox account for:
   - Lambda function `ndx-quicksight-data-generator-{region}`
   - S3 bucket `ndx-quicksight-data-{account-id}-{region}`
   - IAM role `ndx-quicksight-data-generator-{region}`
   - QuickSight dashboard `ndx-council-dashboard-{region}`
4. Open the QuickSight console and verify the council metrics dashboard renders correctly
